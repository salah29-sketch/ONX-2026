...
<div class="card db-card mb-4">
    <div class="db-card-header db-card-header-toggle d-flex justify-content-between align-items-center" data-toggle="collapse" data-target="#collapse-booking-details" role="button" aria-expanded="true">
        <span>بيانات الحجز</span>
        <span class="d-flex align-items-center" onclick="event.stopPropagation();">
            <button type="button" class="btn btn-sm btn-outline-primary border-0 mr-2 js-booking-edit-toggle" title="تعديل" id="btn-booking-edit">
                <i class="fas fa-pen"></i>
            </button>
            <i class="fas fa-chevron-down db-collapse-icon"></i>
        </span>
    </div>
    <div id="collapse-booking-details" class="card-body db-card-body collapse show">
        {{-- وضع العرض (افتراضي) ─── --}}
        <div id="booking-details-view" class="db-detail-grid">
            ...
            <div class="db-detail-item">
                ...
                <div class="db-detail-value">{{ $booking->service_type === 'event' ? 'حفلات' : 'إعلانات' }}</div>
            </div>
            <div class="db-detail-item">
                <div class="db-detail-label">الباقة المختارة</div>
                <div class="db-detail-value">
                    @if($booking->service_type === 'event'){{ $booking->eventPackage->name ?? '—' }}
                    @else{{ $booking->adPackage->name ?? '—' }}
                    @endif
                </div>
            </div>
            <div class="db-detail-item">
                <div class="db-detail-label">السعر الإجمالي (DA)</div>
                <div class="db-detail-value">{{ $booking->total_price ? number_format($booking->total_price, 0) : '—' }}</div>
            </div>
            @if($booking->service_type === 'event')
                ...
                <div class="db-detail-item">
                    ...
                    <div class="db-detail-value">{{ $booking->custom_event_location ?: ($booking->eventLocation->name ?? '—') }}</div>
                </div>
                @if($booking->custom_event_location)
                <div class="db-detail-item">
                    <div class="db-detail-label">مكان مخصص</div>
                    <div class="db-detail-value">{{ $booking->custom_event_location }}</div>
                </div>
                @endif
            @else
                <div class="db-detail-item">
            ...
            <div class="db-detail-item">
                ...
                <div class="db-detail-value">{{ $statusList[$booking->status]['label'] ?? $booking->status }}</div>
            </div>
        ...
        <div id="booking-details-edit" class="d-none">
            <form action="{{ route('admin.bookings.updateDetails', $booking->id) }}" method="POST">
                ...
                <div class="db-detail-grid">
                    ...
                    <div class="db-detail-item">
                        ...
                        <div class="db-detail-value">
                            @if($booking->service_type === 'event')
                                <select name="package_id" class="form-control form-control-sm db-input">
                                    <option value="">— اختر —</option>
                                    @foreach($eventPackages ?? [] as $id => $name)
                    ...
                    <div class="db-detail-item">
                        <div class="db-detail-label">السعر الإجمالي (DA)</div>
                        <div class="db-detail-value">
                            <input type="number" name="total_price" value="{{ $booking->total_price }}" min="0" step="1" placeholder="0" class="form-control form-control-sm db-input">
                        </div>
                    </div>
                    @if($booking->service_type === 'event')
                        <div class="db-detail-item">
                            <div class="db-detail-label">تاريخ الحفل</div>
                            <div class="db-detail-value">
                                <input type="date" name="event_date" value="{{ $booking->event_date?->format('Y-m-d') }}" class="form-control form-control-sm db-input">
                            </div>
                        </div>
                        <div class="db-detail-item">
                            <div class="db-detail-label">مكان الحفل</div>
                            <div class="db-detail-value">
                                <select name="event_location_id" class="form-control form-control-sm db-input">
                                    <option value="">— اختر —</option>
                                    @foreach($eventLocations as $id => $name)
                                        <option value="{{ $id }}" {{ (string)$booking->event_location_id === (string)$id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                        ...
                        <div class="db-detail-item">
                            <div class="db-detail-label">مكان مخصص</div>
                            <div class="db-detail-value">
                                <input type="text" name="custom_event_location" value="{{ $booking->custom_event_location }}" class="form-control form-control-sm db-input">
                            </div>
                        </div>
                    @endif
                    <div class="db-detail-item">
                    ...
                    <div class="db-detail-item">
                        <div class="db-detail-label">الحالة</div>
                        <div class="db-detail-value">
                            <select name="status" class="form-control form-control-sm db-input">
                                @foreach($statusList as $value => $info)
                                    <option value="{{ $value }}" {{ $booking->status === $value ? 'selected' : '' }}>{{ $info['label'] }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="db-detail-item" style="grid-column:1/-1;">
                        <div class="db-detail-label">الملاحظات</div>
                        <div class="db-detail-value">
                            <textarea name="notes" class="form-control db-input" rows="3">{{ $booking->notes }}</textarea>
                        </div>
                    </div>
                    <div class="db-detail-item">
                        <div class="db-detail-label">تاريخ الإنشاء</div>
...
<div class="card db-card mt-4">
    <div class="db-card-header db-card-header-toggle d-flex justify-content-between align-items-center" data-toggle="collapse" data-target="#collapse-booking-photos" role="button" aria-expanded="true">
        <span>صور الحجز (العميل يشاهدها ويختار حتى 200 مميزة للطباعة)</span>
        <i class="fas fa-chevron-down db-collapse-icon"></i>
    </div>
    <div id="collapse-booking-photos" class="card-body db-card-body collapse show">
        ...
        <div class="booking-photos-upload mb-4">
            <form action="{{ route('admin.bookings.photos.store') }}" method="POST" enctype="multipart/form-data" class="form-inline flex-wrap align-items-end">
                @csrf
                <input type="hidden" name="booking_id" value="{{ $booking->id }}">
                <div class="form-group mr-3 mb-2">
                    <label class="db-label mb-1 d-block">رفع صور</label>
                    <input type="file" name="photos[]" multiple accept="image/*" class="form-control">
                </div>
                <button type="submit" class="btn db-btn-primary mb-2">
        ...
        @if($photosPaginated->isNotEmpty())
            <div class="booking-photos-meta mb-3 d-flex justify-content-between align-items-center flex-wrap gap-2">
                    ...
                    <div class="col-6 col-sm-4 col-md-3 col-lg-2 mb-3">
                        <div class="booking-photo-card">
                            ...
                            <div class="booking-photo-actions">
                                <a href="{{ asset($p->path) }}" class="btn btn-sm btn-light js-booking-lightbox-open" title="تكبير">
                                    <i class="fas fa-expand"></i>
                                </a>
                                <form action="{{ route('admin.bookings.photos.destroy', $p) }}" method="POST" class="d-inline" onsubmit="return confirm('حذف هذه الصورة؟');">
                                    @csrf
                                    @method('DELETE')
...
<div class="card db-card mt-4 border-success">
    <div class="db-card-header db-card-header-toggle bg-success text-white d-flex justify-content-between align-items-center" data-toggle="collapse" data-target="#collapse-client-photos" role="button" aria-expanded="true">
        <span>ما اختاره العميل للطباعة ({{ $clientSelectedPhotos->count() }})</span>
        <i class="fas fa-chevron-down db-collapse-icon"></i>
    </div>
    <div id="collapse-client-photos" class="card-body db-card-body collapse show">
        <div class="row">
...
.booking-photo-actions .btn-light { color: #334155; }
