...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route("admin.appointments.store") }}" method="POST" enctype="multipart/form-data">
            ...
            <label for="event_location_id">مكان الحفل</label>
                <select name="event_location_id" id="event_location_id" class="form-control">
            @foreach($event_locations as $id => $name)
                    <option value="{{ $id }}" {{ old('event_location_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                @endforeach
                ...
                <label for="custom_event_location"> أدخل عنوان المكان  </label>
                <input type="text" name="custom_event_location" id="custom_event_location" class="form-control" value="{{ old('custom_event_location') }}">
            </div>
            ...
            <div class="form-group {{ $errors->has('deposit') ? 'has-error' : '' }}">
                <label for="deposit">{{ trans('cruds.appointment.fields.deposit') }}</label>
                <input type="number" id="deposit" name="deposit" class="form-control" value="{{ old('deposit', isset($appointment) ? $appointment->deposit : '') }}" step="0.01">
...
<script>
    ...
    function toggleCustomLocationField() {
        var selected = document.getElementById('event_location_id').value;
        var customInput = document.getElementById('custom_event_location').closest('.form-group');
        if (selected === 'other') {
            customInput.style.display = 'block';
        } else {
            customInput.style.display = 'none';
            document.getElementById('custom_event_location').value = '';
        }
    }

    document.addEventListener('DOMContentLoaded', function () {
        toggleCustomLocationField();
        document.getElementById('event_location_id').addEventListener('change', toggleCustomLocationField);
    });
