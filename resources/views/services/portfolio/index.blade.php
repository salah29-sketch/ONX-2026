        ...
        <div id="portfolioGrid" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            @foreach($items as $index => $item)
                @php
                    ...
                    $previewVideoUrl = !empty($item->preview_video_path) ? asset('storage/' . $item->preview_video_path) : '';
                ...
                >
                    <button
                        type="button"
                        class="block w-full text-left relative h-[320px] md:h-[380px] xl:h-[420px] portfolio-open-btn"
                    >
                        @if($imageUrl)
                            <img
                                src="{{ $imageUrl }}"
                                alt="{{ $item->title }}"
                                class="portfolio-image absolute inset-0 w-full h-full object-cover transition duration-700 ease-out group-hover:scale-105"
                            >
                        @endif

                        @if($previewVideoUrl)
                            <video
                                class="portfolio-preview-video absolute inset-0 w-full h-full object-cover opacity-0 group-hover:opacity-100 transition duration-500"
                                muted
                                loop
                                playsinline
                                preload="metadata"
                        ...
                        <div class="absolute inset-0 bg-gradient-to-t from-black/85 via-black/20 to-transparent group-hover:from-black/90 transition duration-500"></div>
    ...
    <div class="absolute inset-0 flex flex-col">
        ...
        <div class="flex-1 relative overflow-hidden">
            <button
                id="prevItem"
                type="button"
                class="absolute left-4 md:left-8 top-1/2 -translate-y-1/2 z-10 w-12 h-12 rounded-full border border-white/15 bg-black/40 text-white hover:bg-white hover:text-black transition"
            ...
            <button
                id="nextItem"
                type="button"
                class="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 z-10 w-12 h-12 rounded-full border border-white/15 bg-black/40 text-white hover:bg-white hover:text-black transition"
            ...
            <div class="h-full max-w-7xl mx-auto px-6 md:px-10 py-8 md:py-10 grid grid-cols-1 lg:grid-cols-[minmax(0,1.4fr)_380px] gap-8 items-center">
                <div class="relative rounded-[2rem] overflow-hidden border border-white/10 bg-white/5 min-h-[300px] md:min-h-[520px] flex items-center justify-center">
                    <div id="viewerMedia" class="w-full h-full"></div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function stopAllPreviewVideos() {
            document.querySelectorAll('.portfolio-preview-video').forEach(video => {
                video.pause();
                video.currentTime = 0;
            });
        }
        ...
        cards.forEach(card => {
            const video = card.querySelector('.portfolio-preview-video');
            if (!video) return;
        ...
        function buildViewerMedia(card) {
            ...
            if (mediaType === 'youtube' && youtubeId) {
                return `
                    <div class="w-full h-full aspect-video">
                        <iframe
                            class="w-full h-full min-h-[300px] md:min-h-[520px]"
            ...
            if (mediaType === 'youtube' && previewVideo) {
                return `
                    <video
