...
<style>
/* ══════════════════════════════════════════
   ONX CLIENT PORTAL — Booking Detail (خلفية فاتحة)
══════════════════════════════════════════ */
        ...
        <div class="steps">
            ...
            <div class="step {{ $step >= 1 ? ($step > 1 ? 'done' : 'active') : '' }}">
                <div class="step-circle">
                    @if($step > 1) <i class="bi bi-check-lg"></i>
                    @else 1 @endif
                </div>
                <div class="step-label">استلام<br>الطلب</div>
            </div>
            <div class="step-line {{ $step > 1 ? 'done' : ($step === 1 ? 'active' : '') }}"></div>
            {{-- Step 2 --}}
            <div class="step {{ $step >= 2 ? ($step > 2 ? 'done' : 'active') : '' }}">
                <div class="step-circle">
                    @if($step > 2) <i class="bi bi-check-lg"></i>
                    @else 2 @endif
                </div>
                <div class="step-label">تأكيد<br>الحجز</div>
            </div>
            <div class="step-line {{ $step > 2 ? 'done' : ($step === 2 ? 'active' : '') }}"></div>
            {{-- Step 3 --}}
            <div class="step {{ $step >= 3 ? ($step > 3 ? 'done' : 'active') : '' }}">
                <div class="step-circle">
                    @if($step > 3) <i class="bi bi-check-lg"></i>
                    @else 3 @endif
                </div>
                <div class="step-label">قيد<br>التنفيذ</div>
            </div>
            <div class="step-line {{ $step > 3 ? 'done' : ($step === 3 ? 'active' : '') }}"></div>
            {{-- Step 4 --}}
            <div class="step {{ $step >= 4 ? 'done' : '' }}">
                <div class="step-circle">
                    @if($step >= 4) <i class="bi bi-check-lg"></i>
                    @else 4 @endif
                </div>
    ...
    <div class="portal-grid">
                ...
                <div class="panel-body">
                    <div class="info-grid">
                        ...
                        <div class="info-item">
                            ...
                            <div class="info-value">
                                @if($meta['packagePrice'])
                                    {{ number_format($meta['packagePrice'], 0) }} DA
                                @else —
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="info-item" style="margin-top:12px;">
                        ...
                        <div class="info-value" style="font-size:13px;font-weight:500;line-height:1.7;color:rgba(255,255,255,.7);">
                            {{ $booking->notes }}
                        </div>
                    </div>
                    @endif
                    ...
                    @else
                        <div class="file-list">
                            ...
                            <a href="{{ route('client.files.download', $file->id) }}"
                               class="file-row" download>
                                <div class="file-icon-wrap"
                                     style="background:{{ $file->typeColor() }}1a;color:{{ $file->typeColor() }};">
        ...
        <div>
                    ...
                    <div class="pay-amounts">
                        <div class="pay-amount-box">
                            ...
                            <div class="pay-amount-value" style="color:#1f2937;">
                                ...
                                <span style="font-size:11px;font-weight:700;color:var(--muted);">DA</span>
                            </div>
                        </div>
                        <div class="pay-amount-box">
            ...
            <div class="panel">
                ...
                <div class="panel-body">
                    <a href="{{ route('client.bookings.invoice', $booking->id) }}"
                       class="invoice-btn" target="_blank">
                        <i class="bi bi-download"></i>
                        تحميل الفاتورة PDF
                    </a>
                    <a href="{{ route('client.bookings.summary', $booking->id) }}"
                       class="invoice-btn mt-3" style="background:rgba(96,165,250,.1);border-color:rgba(96,165,250,.25);color:#60a5fa;" target="_blank">
                        <i class="bi bi-printer"></i>
                        طباعة ملخص الحجز
                    </a>
                    <p style="font-size:11px;color:var(--muted);margin-top:10px;text-align:center;line-height:1.7;">
            ...
            <div class="panel">
                ...
                <div class="panel-body" style="display:flex;flex-direction:column;gap:10px;">
                    @if($booking->photos->isNotEmpty())
                    <a href="{{ route('client.project-photos.booking', $booking->id) }}"
                       style="display:flex;align-items:center;gap:10px;padding:12px 14px;border-radius:14px;background:#f9fafb;border:1px solid var(--border);color:#1f2937;text-decoration:none;font-size:13px;font-weight:700;transition:.2s;">
                    ...
                    <a href="{{ route('client.review.create') }}"
                       style="display:flex;align-items:center;gap:10px;padding:12px 14px;border-radius:14px;background:rgba(245,166,35,.08);border:1px solid rgba(245,166,35,.2);color:var(--gold);text-decoration:none;font-size:13px;font-weight:700;transition:.2s;">
